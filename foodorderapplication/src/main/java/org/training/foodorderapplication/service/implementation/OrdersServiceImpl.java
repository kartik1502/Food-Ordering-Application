...
public class OrdersServiceImpl implements OrdersService {
	...
	public ResponseDto order(int userId, List<FoodQuantityDto> quantityDtos) {
		 ...
		 Optional<Users> users = userService.findById(userId);
		    ...
		    // Fetch all the vendors and store them in a map with vendorId as key
		    Map<Integer, Vendor> vendorMap = vendorService.findAll().stream()
		            .collect(Collectors.toMap(Vendor::getVendorId, Function.identity()));

		    // Get a set of all foodIds to fetch all food items in one query
		    Set<Integer> foodIds = quantityDtos.stream().map(FoodQuantityDto::getFoodId).collect(Collectors.toSet());

		    // Fetch all food items in one query and store them in a map with foodId as key
		    Map<Integer, FoodItem> foodItemMap = foodItemService.findByFoodItemIdAndVendors(foodIds, vendorMap.values()).stream()
		            .collect(Collectors.toMap(FoodItem::getFoodItemId, Function.identity()));
		    ...
		    quantityDtos.forEach(e -> {
		        ...
		        if (foodItem == null) {
		            ...
		            throw new NoSuchFoodExists("Food with food Id: " + e.getFoodId() + " is not present for vendor Id: " + vendor.getVendorId());
		        }
		        ...
		        if (foodItem.getQuantity() >= e.getQuantity()) {
		            ...
		            responses.add("Food with food Id:" + foodItem.getFoodItemId() + " has only " + foodItem.getQuantity() + " quantities");
		        }
