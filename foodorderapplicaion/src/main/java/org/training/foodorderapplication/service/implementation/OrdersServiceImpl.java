	...
	public List<OrdersDto> purchaseHistory(int userId, String filterType) {
		...
		if (filterType.equalsIgnoreCase("week")) {
			...
			if (ordersDtos.isEmpty()) {
				throw new NoSearchHistoryAvailable("User with user Id:"+userId+" does not have any purchases in the last week");
			}
			return ordersDtos;
			...
			if (ordersDtos.isEmpty()) {
				throw new NoSearchHistoryAvailable("User with user Id:"+userId+" does not have any purchases in the last month");
			}
			return ordersDtos;
		}
	}
	...
	public ResponseDto order(int userId, List<FoodQuantityDtoCreate> quantityDtos) {
		...
		quantityDtos.forEach(e -> {
			...
			if (foodItem.get().getQuantity() >= e.getQuantity()) {
				...
				responses.add("Food with food Id:" + foodItem.get().getFoodItemId() + " has only "
						+ foodItem.get().getQuantity() + " quantities");
			}
